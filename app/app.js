...
function showLoginPage() {
  ...
  fpPromise
  .then(fp => fp.get())
  .then(result => setCookie("fp", result.visitorId, 1))
...
function showHomePage() {
  ...
  // если до истечения токена осталось менее 10 секунд - делаем запрос на обновление
  if(getDiffInSeconds(result.exp) < 10) {
...
function refreshAccessTokens() {
  ...
  $.post("api/v1/auth/refresh_tokens.php").done(function(result) {
    // сохраняем новый JWT в cookie 
    setCookie("jwt", result.jwt, 1);
    return true;
  })
    ...
    if(getDiffInSeconds(result.exp) < 10) {
      if(refreshAccessTokens())
        ...
        <div class="card pb-2 mt-4 mb-2">
            ...
            <div class="card-body">
                <h5 class="card-title">Вы вошли в систему как ${user_role_ru}.</h5>
                <p class="card-text">Токен истекает через ${ getDiffInSeconds(result.exp) } сек. Вы не сможете получить доступ к домашней странице и страницам учетной записи, если вы не вошли в систему.</p>
            </div>
        </div>
...
// расчитывает временную разницу в секундах
function getDiffInSeconds(exp) {
